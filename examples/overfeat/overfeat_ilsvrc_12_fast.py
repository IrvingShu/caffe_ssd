...
def OverFeatBody(net, from_layer, for_training=True):
  ...
  net.conv2 = L.Convolution(net.pool1, kernel_size=k_conv2, stride=s_conv2, num_output=d_conv2, pad=p_conv2, 
                            bias_term=True, weight_filler=dict(type='gaussian',std=0.01), bias_filler=dict(type='constant',std=0), 
                            param=[dict(lr_mult=1,decay_mult=1), dict(lr_mult=2,decay_mult=0)])
  ...
  net.conv3 = L.Convolution(net.pool2, kernel_size=k_conv3, stride=s_conv3, num_output=d_conv3, pad=p_conv3, 
                            bias_term=True, weight_filler=dict(type='gaussian',std=0.01), bias_filler=dict(type='constant',std=0), 
                            param=[dict(lr_mult=1,decay_mult=1), dict(lr_mult=2,decay_mult=0)])
...
batch_size = 32
accum_batch_size = train_batch_size
...
OverFeatBody(train_net, from_layer='data', for_training=True)
...
OverFeatBody(test_net, from_layer='data', for_training=False)
